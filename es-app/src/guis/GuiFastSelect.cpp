...
bool GuiFastSelect::input(InputConfig* config, Input input)
{
	...
	if(config->isMappedTo("arriba", input))
	{
		if(input.value != 0)
			setScrollDir(-1);
		else
		...
		return true;
	}else if(config->isMappedTo("abajo", input))
	{
		if(input.value != 0)
			setScrollDir(1);
		else
		...
		return true;
	}else if(config->isMappedTo("izquierda", input) && input.value != 0)
	{
		mSortId = (mSortId + 1) % FileSorts::SortTypes.size();
		updateSortText();
		return true;
	}else if(config->isMappedTo("derecha", input) && input.value != 0)
	{
		mSortId--;
		if(mSortId < 0)
